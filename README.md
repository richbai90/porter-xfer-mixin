...
[![Build Status](https://dev.azure.com/getporter/porter/_apis/build/status/xfer?branchName=main)](https://dev.azure.com/getporter/porter/_build/latest?definitionId=13&branchName=main)
...
1. Rename the `cmd/xfer` and `pkg/xfer` directories to `cmd/YOURMIXIN` and
   `pkg/YOURMIXIN`.
1. Find the text `github.com/richbai90/xfer/mixin/xfer` in the repository and change it to
   `github.com/YOURNAME/YOURREPO`.
1. Find any remaining `xfer` text in the repository and replace it with `YOURMIXIN`.
...
Edit the `Build` function in `pkg/xfer/build.go`.
...
In the `cmd/xfer` directory, you will find a cli built using [spf13/cobra](https://github.com/spf13/cobra). The CLI contains a go file for each basic capability a Mixin should implement:
...
Each of these command implementations have a corresponding Mixin implementation in the `pkg/xfer` directory. Each of the commands above is wired into an empty implementation in `pkg/xfer` that needs to be completed. In order to build a new Mixin, you need to complete these implementations with the relevant technology. For example, to build a [Cloud Formation](https://aws.amazon.com/cloudformation/) mixin, you might implement the methods in `pkg/xfer` using the [AWS Go SDK](https://docs.aws.amazon.com/sdk-for-go/api/service/cloudformation/).
...
Porter provides a [Context](https://porter.sh/src/pkg/context) package that has helpful mechanisms for accessing the File System using [spf13/afero](https://github.com/spf13/afero). This makes it easy to provide mock File System implementations during testing. The Context package also provides a mechanism to encapsualte stdin, stdout and stderr so that they can easily be passed from `cmd/xfer` code to implementing `pkg/xfer` code.

### Template and Static Asset Handling

The project already includes [Packr V2](https://github.com/gobuffalo/packr/tree/master/v2) for dealing with static files, such as templates or other content that is best modeled outside of a Go file. You can see an example of this in `pkg/xfer/schema.go`.

### Basic Schema

The project provides an implementation of the `xfer schema` command that is mostly functional. To fully implement this for your mixin, you simply need to provide a valid JSON schema. For reference, consult `pkg/xfer/schema/schema.json`.
